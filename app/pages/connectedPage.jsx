...
import {
  ArrowDown, ArrowUp, Wifi, Shield, User,
  LogOut, Palette, Activity, Clock,
} from "lucide-react";
...
const ORB_PRESETS = [
  { id: "blue",   from: "#93c5fd", mid: "#3b82f6", to: "#1e3a8a", ring: "#60a5fa" },
  { id: "cyan",   from: "#a5f3fc", mid: "#06b6d4", to: "#164e63", ring: "#22d3ee" },
  { id: "purple", from: "#d8b4fe", mid: "#8b5cf6", to: "#3b0764", ring: "#a78bfa" },
  { id: "green",  from: "#86efac", mid: "#22c55e", to: "#14532d", ring: "#4ade80" },
  { id: "pink",   from: "#fbcfe8", mid: "#ec4899", to: "#500724", ring: "#f472b6" },
];
...
export function ConnectedPage({ email, onDisconnect }) {
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      setDownload((p) => Math.max(5,  Math.min(200, p + (Math.random() - 0.45) * 14)));
      setUpload  ((p) => Math.max(1,  Math.min(60,  p + (Math.random() - 0.45) * 5)));
      setPing    ((p) => Math.max(8,  Math.min(80,  p + Math.round((Math.random() - 0.5) * 8))));
    ...
    <div className="bg-black w-full h-screen flex flex-col relative overflow-y-auto overflow-x-hidden select-none">
      ...
      <div className="flex items-center justify-between px-5 pt-12 pb-2 z-10">
        <div>
          ...
          <h1 className="text-xl font-bold text-white mt-0.5 capitalize">{profileName}</h1>
        </div>
      ...
      <div className="flex justify-center z-10 mt-1">
        <svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* Outer ring */}
          <g>
            <circle cx="100" cy="100" r="87" stroke={orb.ring} strokeWidth="1" strokeDasharray="3 7" fill="none" opacity="0.4" />
            <circle cx="100" cy="13" r="3"   fill={orb.ring}   filter="url(#cGlow)" opacity="0.8" />
          ...
          {/* Counter ring */}
          <g>
            <circle cx="100" cy="100" r="74" stroke={orb.mid} strokeWidth="1.5" strokeDasharray="5 9 2 9" fill="none" opacity="0.5" />
            <circle cx="100" cy="26" r="4"   fill={orb.from}  filter="url(#cGlow)" opacity="0.9" />
          ...
          {/* Main orb */}
          <circle cx="100" cy="100" r="55" fill="url(#cOrbGrad)" filter="url(#cSoftGlow)" />
          <ellipse cx="88" cy="82" rx="18" ry="11" fill="white" opacity="0.13" transform="rotate(-25 88 82)" />

          {/* Speed text */}
          <text x="100" y="96" textAnchor="middle" fill="white" fontSize="20" fontWeight="bold" fontFamily="monospace">
            {download.toFixed(0)}
          </text>
          <text x="100" y="110" textAnchor="middle" fill="white" fontSize="9" opacity="0.65" fontFamily="sans-serif" letterSpacing="1">
            Mbps
          </text>

          {/* Pulse rings */}
          <circle cx="100" cy="100" r="55" stroke={orb.ring} strokeWidth="1.5" fill="none">
            <animate attributeName="r"            from="55" to="78" dur="2s" repeatCount="indefinite" />
            <animate attributeName="opacity"      from="0.5" to="0" dur="2s" repeatCount="indefinite" />
            <animate attributeName="stroke-width" from="1.5" to="0" dur="2s" repeatCount="indefinite" />
          </circle>
          <circle cx="100" cy="100" r="55" stroke={orb.ring} strokeWidth="1.5" fill="none">
            <animate attributeName="r"            from="55" to="78" dur="2s" begin="1s" repeatCount="indefinite" />
            <animate attributeName="opacity"      from="0.5" to="0" dur="2s" begin="1s" repeatCount="indefinite" />
            <animate attributeName="stroke-width" from="1.5" to="0" dur="2s" begin="1s" repeatCount="indefinite" />
          </circle>
        </svg>
      </div>
      ...
      <div className="flex items-center justify-center gap-8 z-10 -mt-1">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-lg font-bold text-white font-mono leading-none">{download.toFixed(1)}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-lg font-bold text-white font-mono leading-none">{upload.toFixed(1)}</span>
            <span className="text-[10px] text-gray-500">Mbps</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 mt-4 grid grid-cols-3 gap-2 z-10">
        <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-3 flex flex-col gap-1.5">
          ...
          <span className="text-base font-bold text-white font-mono leading-none">{ping}<span className="text-xs font-normal text-gray-500">ms</span></span>
          <span className="text-[10px] text-gray-600 uppercase tracking-wide">Ping</span>
        </div>
        <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-3 flex flex-col gap-1.5">
          <Wifi size={12} className="text-gray-500" />
          <span className="text-base font-bold text-white font-mono leading-none">{dataUsed.toFixed(2)}<span className="text-xs font-normal text-gray-500">GB</span></span>
          <span className="text-[10px] text-gray-600 uppercase tracking-wide">Consumo</span>
        </div>
        <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-3 flex flex-col gap-1.5">
          <Clock size={12} className="text-gray-500" />
          <span className="text-[11px] font-bold text-white font-mono leading-none">{formatTime(connTime)}</span>
          <span className="text-[10px] text-gray-600 uppercase tracking-wide">Tempo</span>
        </div>
      </div>
      ...
      <div className="px-5 mt-3 z-10">
        <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl px-4 py-3 flex items-center gap-3">
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${orb.from}, ${orb.to})` }}
          >
            <User size={17} className="text-white" />
          </div>
          <div className="flex flex-col flex-1 min-w-0">
            <span className="text-sm font-bold text-white capitalize truncate">{profileName}</span>
      ...
      <div className="px-5 mt-3 z-10">
        <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl px-4 py-4">
          <div className="flex items-center gap-1.5 mb-4">
            ...
            <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Personalizar</span>
          </div>

          <div className="flex flex-col gap-4">
            {/* BG preset */}
            <div>
              <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">Fundo</p>
            ...
            {/* Orb preset */}
            <div>
              <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">Bolinha</p>
              <div className="flex gap-2.5">
                {ORB_PRESETS.map((o) => (
                  <button
                    key={o.id}
